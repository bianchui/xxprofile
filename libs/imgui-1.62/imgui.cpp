...
void ImGui::PlotEx(ImGuiPlotType plot_type, ImHistogramWithHitTest& value) {
    ...
    if (value.values_count > 0) {
        ...
        float t0 = 0.0f;
        ImVec2 tp0 = ImVec2( t0, 1.0f - ImSaturate((v0 - value.scale_min) * inv_scale) );                       // Point in the normalized space of our target rectangle
        float histogram_zero_line_t = (value.scale_min * value.scale_max < 0.0f) ? (-value.scale_min * inv_scale) : (value.scale_min < 0.0f ? 0.0f : 1.0f);   // Where does the zero line stands
        ...
        for (int n = 0; n < res_w; n++) {
            ...
            ImVec2 pos0 = ImLerp(inner_bb.Min, inner_bb.Max, tp0);
            ImVec2 pos1 = ImLerp(inner_bb.Min, inner_bb.Max, (plot_type == ImGuiPlotType_Lines) ? tp1 : ImVec2(tp1.x, histogram_zero_line_t));
            ImU32 col = v_hovered == v1_idx ? col_hovered : col_base;
            float w = pos1.x - pos0.x;
            if (w <= 2 && v1_idx == value.hitItem) {
                col = col_selected;
            }
            ...
            if (v1_idx == value.hitItem && w > 2) {
                window->DrawList->AddRect(ImVec2(pos0.x, inner_bb.Min.y), ImVec2(pos1.x, inner_bb.Max.y), col_selected);
            }
